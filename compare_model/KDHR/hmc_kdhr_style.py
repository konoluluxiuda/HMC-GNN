...
# -*- coding: utf-8 -*-
"""
KDHR-style baseline adapted to the HMC-GNN disease-herb setting.

The original KDHR model is a symptom-set-to-herb-set recommender with three
graphs: symptom-herb, symptom-symptom, and herb-herb. This script keeps that
core idea, but replaces the original task with HMC-GNN's disease-to-herb
ranking task and evaluates under the same disease-label-disjoint protocol.
"""
...
def load_dense_feature_stack(
    ...
    print(f"🔹 KDHR-style dense semantic stack: {tuple(dense_features.shape)}")
...
def build_mean_adj(edge_index, num_nodes, device, label):
    ...
    print(f"✅ KDHR-style {label}: edges={edge_index.size(1)}")
    return adj


def build_kdhr_adjs(graph_dir, edge_index, edge_type, num_nodes, same_type_mode, device):
    ...
    dh_edges = select_edges(edge_index, edge_type, ids(interaction_names))
    hh_edges = select_edges(edge_index, edge_type, ids(hh_names))
    dd_edges = select_edges(edge_index, edge_type, ids(dd_names))

    print(f"✅ KDHR-style same-type mode: {same_type_mode}")
    print(f"   D-H relations: {interaction_names}")
    print(f"   D-D relations: {dd_names}")
    print(f"   H-H relations: {hh_names}")

    return {
        "dh": build_mean_adj(dh_edges, num_nodes, device, "D-H graph"),
        "dd": build_mean_adj(dd_edges, num_nodes, device, "D-D graph"),
        "hh": build_mean_adj(hh_edges, num_nodes, device, "H-H graph"),
    }


def add_global_context_adj(graph_dir, edge_index, edge_type, num_nodes, adjs, device):
    """
    Add the same external/global side-information edge pool used by HMC-GNN's
    global branch, but aggregate it with a plain relation-agnostic GCN layer.

    This controls for data availability while keeping KDHR's core difference:
    ordinary graph convolution and additive multi-graph fusion instead of
    relation-aware RGCN with local/global/semantic branch learning.
    """
    relation_to_id = load_relation_name_to_id(graph_dir)
    ...
    if not external_relation_ids:
        raise RuntimeError("No external/global relation ids found for KDHR controlled baseline.")

    external_edges = select_edges(edge_index, edge_type, external_relation_ids)
    external_names = sorted(
        relation for relation in relation_to_id if relation not in LOCAL_RELATION_NAMES
    )
    print("✅ KDHR-style controlled external/global context enabled.")
    print(f"   External relations: {external_names}")
    adjs["global"] = build_mean_adj(external_edges, num_nodes, device, "external/global graph")
    return adjs
...
class KDHRStyleDiseaseHerb(nn.Module):
    def __init__(
        self,
        num_nodes,
        embedding_dim,
        ...
        self.dd_layer = MeanGCNLayer(embedding_dim, dropout)
        self.hh_layer = MeanGCNLayer(embedding_dim, dropout)
    ...
    def encode(self, adjs):
        ...
        dh_disease = x
        dh_herb = x
        dh_disease_history = [dh_disease]
        dh_herb_history = [dh_herb]
        for disease_layer, herb_layer in zip(self.dh_disease_layers, self.dh_herb_layers):
            dh_disease = disease_layer(dh_disease, adjs["dh"])
            dh_herb = herb_layer(dh_herb, adjs["dh"])
            dh_disease_history.append(dh_disease)
            dh_herb_history.append(dh_herb)

        dh_disease = torch.stack(dh_disease_history, dim=0).mean(dim=0)
        dh_herb = torch.stack(dh_herb_history, dim=0).mean(dim=0)
        dd = self.dd_layer(x, adjs["dd"])
        hh = self.hh_layer(x, adjs["hh"])
        if self.global_layer is not None:
            ...
            global_context = self.global_layer(x, adjs["global"])
        else:
            global_context = 0.0

        disease_z = self.disease_mlp(dh_disease + dd + self.global_context_weight * global_context)
        herb_z = self.herb_mlp(dh_herb + hh + self.global_context_weight * global_context)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run KDHR-style baseline on HMC-GNN disease-herb data.")
    parser.add_argument(
        "--graph-dir",
    ...
    parser.add_argument("--same-type-mode", choices=["cooccurrence", "gene_jaccard", "both"], default="both")
    parser.add_argument(
        "--global-context",
        choices=["none", "external"],
        default="external",
        help=(
            "Use HMC-GNN's external/global side-information edges with a plain "
            "relation-agnostic GCN layer. This controls data availability while "
            "preserving KDHR's core ordinary multi-graph GCN idea."
        ),
    )
    parser.add_argument("--global-context-weight", type=float, default=1.0)
    ...
    parser.add_argument("--save-path", default="checkpoints/best_kdhr_style.pt")
...
def main():
    ...
    print("\n" + "=" * 50)
    print("KDHR-style Disease-Herb Baseline")
    print("=" * 50)
    print(f"Graph dir:      {graph_dir}")
    print(f"Feature dir:    {feature_dir}")
    print(f"Device:         {device}")
    print("Protocol:       same disease-label-disjoint split as HMC-GNN")
    print("Core graph:     D-H + D-D + H-H multi-graph GCN")
    print(f"Same-type mode: {args.same_type_mode}")
    print(f"Global context: {args.global_context} (weight={args.global_context_weight})")
    ...
    if args.global_context == "external":
        adjs = add_global_context_adj(
            graph_dir,
            edge_index,
            edge_type,
            data_manager.num_nodes,
    ...
    model = KDHRStyleDiseaseHerb(
        ...
        global_context_weight=args.global_context_weight,
    ).to(device)
    print(f"✅ KDHR-style input streams: {', '.join(model.stream_names)}")
    ...
    print_results("Final KDHR-style Baseline Test Results", test_results, k_list)
